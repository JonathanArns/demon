    ...
    for kind in ["remote", "client"]:
        ...
        for proto in protocols:
            ...
            plt.bar(
                ...
                yerr=([0 for x in range(len(ops))], [proto_df[f"{op}_{kind}_p95_latency"].iloc[0] for op in ops]),
                width=bar_width,
                label=LABELS[proto],
                capsize=3.0,
                color=COLORS[proto],
        ...
        plt.legend()
        plt.savefig(os.path.join(dir_path, f"non-neg-counter-{kind}-latency-bars.png"), dpi=300)
