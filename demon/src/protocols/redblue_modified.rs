...
/// A version of RedBlue consistency that uses a semi-causal order instead.
pub struct RedBlueModified<O: Operation> {
...
#[async_trait]
impl<O: Operation> MsgHandler<Message> for RedBlueModified<O> {
...
impl<O: Operation> RedBlueModified<O> {
    ...
    /// Instead of running the normal barrier for RedBlue here, we instead choose the snapshot
    /// like in semi-serializability.
    async fn snapshot_barrier(&self) -> Snapshot {
        self.quorum_replicated_snapshot.read().await.clone()
    }
